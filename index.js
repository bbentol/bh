...
async function start() {
	...
	client.on('message-new', async (mek) => {
		try {
            ...
            if (args.length !== 2) return client.reply(from, 'Maksimal 2 kata\n\nContoh: #phub kata1 kata2', id)
            if (q.length > 20) return client.reply(from, 'teks terlalu panjang, minimal 20 huruf kak', id)
            client.sendFileFromUrl(from, 'https://api.vhtear.com/pornlogo?text1=' + args[0] + '&text2=' + args[1] + '&apikey=' + apiVhtear)
            .catch((err) => {
                client.reply(from, 'Sepertinya server sedang error!', id)
                client.sendText(ownerNumber, 'Sepertinya phub sedang error : ' + err);
            })
            }
